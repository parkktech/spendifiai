...
export default function GoogleLoginButton({ label = 'Continue with Google' }: { label?: string }) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="group relative w-full flex items-center justify-center gap-3 px-4 py-3 mt-4 bg-white border border-sw-border rounded-lg text-sm font-medium text-sw-text hover:bg-gray-50 hover:border-sw-border-strong hover:shadow-md active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {loading ? (
        <>
