...
class CaptchaService
{
    ...
    public function verify(string $token, ?string $action = null, ?string $ip = null): bool
    {
        ...
        try {
            $response = Http::asForm()->post(config('spendwise.captcha.verify_url'), [
                'secret'   => config('spendwise.captcha.secret_key'),
                'response' => $token,
                'remoteip' => $ip,
            ]);
            ...
            $threshold = config('spendwise.captcha.threshold', 0.5);
