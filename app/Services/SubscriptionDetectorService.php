...
class SubscriptionDetectorService
{
     ...
     * 2× the subscription frequency (e.g., >60 days for monthly, >180 days for quarterly).
     *
     * This avoids false positives — we only flag subscriptions where it looks like
     * charges have actually stopped, not ones that are still actively billing.
     */
    protected function detectUnusedSubscriptions(int $userId): void
    {
        ...
        foreach ($subscriptions as $sub) {
            $maxGapDays = match ($sub->frequency) {
                'weekly' => 21,       // 3× weekly interval
                'monthly' => 60,      // 2× monthly interval
                'quarterly' => 180,   // 2× quarterly interval
