...
class Transaction extends Model
{
    ...
    protected $fillable = [
        ...
        'tax_deductible', 'tax_category', 'donation_note', 'review_status',
        'is_subscription', 'matched_order_id', 'is_reconciled',
    ];
    ...
    public function aiQuestion(): HasOne
    {
        return $this->hasOne(AIQuestion::class);
    }
    ...
    // Query scopes
    public function scopeDeductible($q)
    {
        return $q->where('tax_deductible', true);
    }

    public function scopeBusiness($q)
    {
        return $q->where('account_purpose', AccountPurpose::Business);
    }

    public function scopePersonal($q)
    {
        return $q->where('account_purpose', AccountPurpose::Personal);
    }

    public function scopeNeedsReview($q)
    {
        return $q->whereIn('review_status', [ReviewStatus::NeedsReview, ReviewStatus::PendingAI, ReviewStatus::AIUncertain]);
    }

    public function scopeByCategory($q, string $cat)
    {
        return $q->where(fn ($q) => $q->where('user_category', $cat)->orWhere('ai_category', $cat));
    }

    public function scopeSpending($q)
    {
    ...
    public function scopeDonation($q)
    {
        return $q->where(fn ($q) => $q->where('user_category', 'Charity & Donations')
    ...
    public function getIsDonationAttribute(): bool
    {
        return ($this->user_category ?? $this->ai_category) === 'Charity & Donations';
    }
}
