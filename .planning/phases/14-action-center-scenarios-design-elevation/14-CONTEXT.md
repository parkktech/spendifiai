...
### D17 — AI COST DISCIPLINE (BINDING on every Phase 14 plan/task; added post-definition)
Owner: cheapest capable model, fewest possible calls, stale users cost $0 — the site must not rack up AI bills pre-revenue. Concretely for this phase: (1) TEMPLATE-FIRST, CLAUDE-LAST — every standard output (gap questions, probe wording from the distillations' verbatim trees, standard finding/checklist narrations = finding type + engine numbers) uses deterministic templates with ZERO AI calls; Claude only for genuinely bespoke narrative. (2) Per-call-site model config (`services.anthropic.model_<purpose>`, global default): narration/wording → claude-haiku-4-5; extraction stays claude-sonnet-4-6; categorization → Haiku behind the existing confidence-threshold safety net. (3) Extend the 28-day activity gate (commit 4e75c46 precedent) to ALL AI-triggering paths — inactive users get no AI calls; lazy re-warm on return. (4) No automatic mass backfills. (5) Per-service daily call counters surfaced in the Admin drawer + config daily budget caps (jobs skip gracefully over budget). Full text: enhanced-profile-integration-notes.md Decision 17.
