...
Output: A bootable Laravel 12 application at /var/www/html/ledgeriq with all existing code integrated and database migrated.
</objective>
...
The current directory /var/www/html/ledgeriq has planning files and existing-code/. We need to create a Laravel project HERE (not in a subdirectory). The approach:
1. Back up the existing planning artifacts: `cp -r .planning /tmp/ledgeriq-planning && cp -r existing-code /tmp/ledgeriq-existing && cp CLAUDE.md /tmp/ledgeriq-claude.md && cp -r docs /tmp/ledgeriq-docs 2>/dev/null; cp TASKS.md /tmp/ledgeriq-tasks.md 2>/dev/null`
2. Create the Laravel project in a temp location: `composer create-project laravel/laravel /tmp/ledgeriq-fresh --prefer-dist`
3. Copy the fresh Laravel project files into /var/www/html/ledgeriq (without overwriting .git, .planning, existing-code, CLAUDE.md, docs): Use rsync or careful cp to bring in the Laravel scaffold.
...
6. `php artisan env` (or check .env) shows DB_CONNECTION=pgsql
  ...
  <done>
Laravel 12 project exists at /var/www/html/ledgeriq with React starter kit installed, all PHP and JS dependencies resolved, .env configured for PostgreSQL + Redis, and frontend assets build successfully.
  </done>
</task>
...
PGPASSWORD=soyou812 psql -U root -h 127.0.0.1 -c "CREATE DATABASE ledgeriq_dev;" 2>/dev/null || true
...
```bash
find /var/www/html/ledgeriq/app -name "*:Zone.Identifier" -delete
find /var/www/html/ledgeriq/database -name "*:Zone.Identifier" -delete
find /var/www/html/ledgeriq/config -name "*:Zone.Identifier" -delete
find /var/www/html/ledgeriq/routes -name "*:Zone.Identifier" -delete
find /var/www/html/ledgeriq/resources -name "*:Zone.Identifier" -delete
```
...
8. No Zone.Identifier files exist: `find /var/www/html/ledgeriq/app -name "*:Zone*" | wc -l` outputs 0
  </verify>
  <done>
...
<success_criteria>
- Laravel 12 application boots at /var/www/html/ledgeriq
