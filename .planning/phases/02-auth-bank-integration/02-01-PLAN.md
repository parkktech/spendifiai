...
must_haves:
  truths:
    ...
    - "reCAPTCHA v3 middleware protects registration and login endpoints (returns 422 without valid captcha_token when captcha is enabled)"
  artifacts:
  ...
  key_links:
    ...
    - from: "routes/api.php"
      to: "app/Http/Middleware/VerifyCaptcha.php"
      via: "captcha middleware on register and login routes"
      pattern: "middleware.*captcha:(register|login)"
---
...
<task type="auto">
  <name>Task 2: Add reCAPTCHA middleware and rate limiting to auth routes, add email verification route</name>
  <files>routes/api.php</files>
  <action>
The auth routes need reCAPTCHA v3 protection (AUTH-15), rate limiting, and the email verification API route is missing.

1. **Add reCAPTCHA middleware to register route** -- The `VerifyCaptcha` middleware is already registered as alias `'captcha'` in AppServiceProvider (line 51). It accepts an optional action parameter. Add `'captcha:register'` middleware to the register route:
```php
Route::post('/register', [AuthController::class, 'register'])
    ->middleware(['throttle:5,1', 'captcha:register']);
```

2. **Add reCAPTCHA middleware to login route** -- Add `'captcha:login'` middleware to the login route:
```php
...
- POST api/auth/register has both throttle AND captcha middleware
- POST api/auth/login has both throttle AND captcha middleware
...
Run `php artisan route:list --path=api/auth --columns=method,uri,middleware` and verify `captcha:register` appears on register route and `captcha:login` appears on login route.
...
Run `php artisan route:list --path=api/v1/webhooks` and confirm POST api/v1/webhooks/plaid exists (will show even if controller class doesn't exist yet -- route registration is separate from class loading in route:list).
  ...
  <done>All auth routes have rate limiting. reCAPTCHA v3 middleware (`captcha:register`, `captcha:login`) applied to register and login routes per AUTH-15. Email verification API route registered. Webhook route registered (controller created in Plan 03). Change-password route has throttle:5,1.</done>
</task>
...
Verify reCAPTCHA middleware blocks requests without captcha token (when enabled). Check the config:
```bash
php artisan tinker --execute="echo config('spendwise.captcha.enabled') ? 'ENABLED' : 'DISABLED';"
```
If captcha is enabled, test that register without captcha_token returns 422:
```bash
curl -s -o /dev/null -w "%{http_code}" -X POST http://localhost:8099/api/auth/register \
  -H "Content-Type: application/json" \
  -H "Accept: application/json" \
  -d '{"name":"Test","email":"captcha-test@example.com","password":"password123","password_confirmation":"password123"}'
```
Expected: 422 (CAPTCHA verification failed). If captcha is disabled in config, the middleware passes through (by design) -- verify this by confirming the VerifyCaptcha middleware checks `config('spendwise.captcha.enabled')` and returns early if false.

Test registration endpoint (with captcha disabled or valid token):
```bash
...
```
  ...
  <done>All 5 auth controllers verified correct. Register returns 201 with token. Login returns 200 with token. Me returns user data. Logout revokes token. reCAPTCHA middleware is wired to register and login routes and returns 422 when captcha is enabled and no token is provided. No code changes needed beyond Task 1 model fix.</done>
</task>
...
3. Registration and login routes have `captcha` middleware applied (AUTH-15)
4. Registration, login, logout, and me endpoints all return expected JSON responses
5. User model $fillable includes all lockout and 2FA fields
6. Rate limiting middleware applied to all public auth endpoints
</verification>
...
- AUTH-15: reCAPTCHA v3 protects registration and login endpoints via captcha middleware (returns 422 without valid token when enabled)
</success_criteria>
