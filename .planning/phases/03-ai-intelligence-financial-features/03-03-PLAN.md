...
<task type="auto">
  <name>Task 2: Copy/fix ProcessOrderEmails job, implement EmailConnectionController, and enable email sync schedule</name>
  <files>
    app/Jobs/ProcessOrderEmails.php
    app/Http/Controllers/Api/EmailConnectionController.php
    routes/console.php
...
2. **Fix sync_status field name**: The EmailConnection model has `status` in $fillable (using ConnectionStatus enum cast), not `sync_status`. But the job uses `sync_status` for tracking sync progress. Check the actual DB migration for email_connections table to see if `sync_status` is a separate column.

   Looking at the migration: the `email_connections` table has a `status` column (via ConnectionStatus enum). There is NO separate `sync_status` column. The job writes `sync_status` which doesn't match.

   Two options: (a) add `sync_status` to the model $fillable and DB, or (b) use the existing `status` column. Since `status` uses the `ConnectionStatus` enum which likely has `active`, `error`, etc. states, and `sync_status` is a temporary state (`syncing`, `completed`, `failed`), it's better to add `sync_status` as a separate tracking field.

   However, to keep this simple and avoid another migration, use the `last_synced_at` timestamp to indicate sync completion, and a try/catch for errors. Replace the `sync_status` usage:
   ```php
   // Instead of: $connection->update(['sync_status' => 'syncing']);
   // Just proceed -- the connection status stays 'active'

   // At end, update last_synced_at:
   $connection->update(['last_synced_at' => now()]);

   // On failure, leave last_synced_at unchanged
   ```

   Remove ALL `sync_status` references from the job. The `last_synced_at` field already exists in the model $fillable and serves the same purpose for tracking.
...
Note: The `callback` method uses inline validation for a single required field (`code`). This is acceptable for an OAuth callback parameter similar to how Plan 02-03 used inline validation for the deleteAccount password field.

**Enable email sync schedule in console.php:**

Read `routes/console.php`. Uncomment or add the email sync schedule:

```php
// ── Sync email accounts for order confirmations (every 6 hours) ──
Schedule::call(function () {
    \App\Models\EmailConnection::where('status', 'active')->each(function ($conn) {
...
```bash
grep -n "gmail_message_id\|gmail_thread_id\|sync_status" app/Jobs/ProcessOrderEmails.php
```
Should show ZERO matches (all replaced with correct names).
...
Should show tax routes (summary, export, send-to-accountant, download).
  ...
  <done>ProcessOrderEmails job copied with column names fixed, sync_status removed, reconciliation wired in. EmailConnectionController implements real Gmail OAuth flow. Email sync schedule runs every 6 hours. Tax export pipeline verified (Python dependencies installed, TaxExportService already complete). All email and tax routes resolve correctly.</done>
</task>
...
5. ProcessOrderEmails uses `email_message_id` and `email_thread_id`
6. EmailConnectionController has real implementations (no 501 stubs)
7. `php artisan schedule:list` shows sync-email-orders
8. `php artisan route:list --path=api/v1/email` shows all email routes
9. `php artisan route:list --path=api/v1/tax` shows all tax routes
</verification>
