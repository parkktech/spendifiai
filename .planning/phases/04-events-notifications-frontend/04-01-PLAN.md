...
must_haves:
  truths:
    ...
    - "Answering an AI question dispatches UserAnsweredQuestion which triggers subscription re-detection as a side effect"
...
<task type="auto">
  ...
  <action>
    ...
    4. **UpdateTransactionCategory** (handles UserAnsweredQuestion):
       - The AIQuestionController already updates the transaction via `TransactionCategorizerService::handleUserAnswer()` (which sets user_category, review_status, expense_type, and tax_deductible on the transaction). This listener MUST NOT duplicate that logic.
       - Instead, this listener handles post-answer side effects only:
         a. Dispatch `SubscriptionDetectorService` for the question's user to re-check subscription patterns now that the transaction is categorized
         b. Log or track that the user answered (for analytics/future use)
