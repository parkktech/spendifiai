...
## Decision 22 — "Straighten it out": preparer-ready organization + user structural guidance (owner, 2026-07-02)

Owner: "Let's have this system help straighten it out for the Tax Preparer and the user — to maximize how they spend and where they earn." Two deliverables from the mixed-reality cleanup (label≠behavior, D21 addendum):
1. **Preparer stream**: the pro-review packet (RPT-05, shipped) gains the cross-account business-activity map — Schedule C classification independent of account, documentation status per deduction, commingling picture explained. The preparer deliverable is the ORGANIZED truth, not raw exports.
2. **User stream**: NEW Action Center item type `organization` — structural hygiene guidance ("route business purchases through [business account]", "consolidate owner transfers", "where to earn/deposit for cleaner records") with benefit lines framed as future-tax-simplicity + audit-cleanliness. Educational; never restructuring advice on entities (that stays gated).
Routing: 14-09 (item type) + 14-11 (preparer-packet upgrade) + funnel Stage C outputs.

## E-FILE CAPTURE (owner question, 2026-07-02 — future milestone, NOT v2.1/v2.2)
Owner asked: "Can I add E-File to this system with the IRS?" Answer recorded: possible, three routes — (a) DIRECT MeF integration: become an Authorized IRS e-file Provider (EFIN via e-Services, suitability checks ~45 days) + IRS software-developer registration + annual ATS certification per form family + per-state e-file registrations — a heavy, season-recurring lift; (b) EMBEDDED PARTNER API (pragmatic path): white-label an existing consumer e-file API (evaluate providers like Column Tax / April) — partner carries most IRS/state compliance, we embed the flow on our data; (c) STAY PRE-FILING (current posture): organized exports (TXF/QuickBooks/pro packet) into preparers/software — the explicit v2.1 Out-of-Scope stands. NOTE: any filing route changes the regulatory tier (IRS Pub 1345, GLBA/4557 safeguards, state registrations) and ends the educational-only liability posture — P13-class legal review is a prerequisite, and the D1-D22 machinery (clean classified data) is exactly the asset that makes an embedded-filing partner integration valuable later. Recommended: revisit as a v3 initiative after v2.1 ships and the ads/debt tiers prove revenue.
